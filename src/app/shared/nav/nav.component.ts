...
export class NavComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.tfingi_megamenu_init();
  }

  tfingi_megamenu_init() {
    $('.megamenu-parent').each(function (index, el) {
      let megamenu_parent = $(this);
      let megamenu = $(megamenu_parent).children('.sub-menu');
      let megamenu_columns_count = megamenu.children('.megamenu-column').length;

      // Get predefined in menu width for megamenu using data-width attribute
      let megamenu_width = megamenu_parent.attr('data-width');
      let megamenu_left = '';
      let megamenu_parent_itemwidth = 0;
      ...
      if (megamenu_width === 'full') {
        ...
        let megamenu_left = parseInt(megamenu_width, 10) / 2 * (-1) + megamenu_parent_itemwidth / 2;
      ...
      let megamenu_parent_offset = $(megamenu_parent).offset();
